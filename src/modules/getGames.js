export default class FetchGamesApi {
  ...
  static getGamesFetch = async => {
    ...
    return fetch('https://free-to-play-games-database.p.rapidapi.com/api/games?category=shooter', options)
  ...
  static gamesRender = async (gamesData) => {
    gamesData.forEach(async (e) => {
      ...
      <div class="content">
       <div class="name-div">
        <h2>${e.title}</h2>
        <span class="likes"><i class="fas fa-heart fa-lg"></i></span>
       </div>
       <span class="number-of-likes">5 Likes</span>
